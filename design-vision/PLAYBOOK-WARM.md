...
- **🔵 F4 — Operational (ĐANG CHẠY, dựng-mới warm-native từ `docs/API.md`):**
  ...
  - **⏳ F4b — Emergency lane** (`/emergency`): `GET /api/cases/emergency` (`?activeOnly`); STUDENT→403; gate sensitivity (`OR:[{isSensitive:false},{assignedToId:me}]`); beat khẩn Rich (Spiced Wine/oxblood). → tag `warm-f4b`.
  - **⏳ F4c — Staff queue** (`/queue`): cases NEW/TRIAGED tái dùng `useCaseList`; signal-gutter sort. + **Gate F4** + tag `warm-f4`. **Search (F4.5) → hoãn F5+** (không có endpoint).
- **Điểm xuất phát (lịch sử):** `design-vision/landing-warm.html` · `login-warm-a.html`.
- **Resume:** `/clear` → "**continue F4b**" (Emergency lane warm-native; xem `docs/API.md` GET `/api/cases/emergency`). Sau F4b → F4c (+Gate F4, tag `warm-f4`) → F5 → F6 → F7. Mỗi phase = 1 context sạch, commit + tag trước clear.
