...
- **Resume:** `/clear` → "**continue F4**" (Operational dựng-mới warm-native từ `docs/API.md`: notifications + audit + emergency-lane + queue + search; cần AppShell/nav — đã ổn định) → F5 → F6 → F7. Mỗi phase = 1 context sạch, commit + tag trước clear.
